...
- Bangumi API 客户端与 AI 陪伴模块已实现并经真实端点验证；AI 尚未部署进 AstrBot 运行时。
- 已下载并校验 Bangumi Archive 的 `2026-09-08` 快照到本地 `data/`；尚未解压、导入或接入运行逻辑。
- 阶段 2 已完成：领域包可构建，薄插件可由真实 AstrBot 加载，假事件集成测试通过。
- 阶段 3 已完成：NapCat/OneBot 真实连接、目标群收发、答案归一化及管理员静默/恢复均已验收。
- 阶段 4 已开始：SQLite 迁移、通用状态存储及游戏/静默持久化代码已完成单元测试；真实跨重启验收尚未完成。
- 已实现 Bangumi API 客户端、SQLite TTL 缓存与安静降级（默认 `api.bgm.tv`，真实网络验证通过；详见 `third-party/bangumi-api.md`）。
- 已实现 AI 陪伴模块（点名/引用触发、有限上下文、30 天本地保留、安静降级），Ark coding 端点真实调用验证通过。
