...
- 已实现事件流与管理 WebUI：SQLite schema v2 事件表（7 天保留）、适配层全链路打点（门控/决策/动作/游戏/跳过含原因）、`apeiria_webui` 只读管理台（127.0.0.1:6186，实时事件流 + 状态 + 限频余量 + 掩码配置）；64 项测试通过。
- 已实现游戏表达层（`AiGamePresenter`）：全部游戏回复措辞经 LLM 生成（full 整句 / lead 引子拼接两种模式），表达上下文按 kind 裁剪防剧透，失败回退模板池；闲聊分诊（游戏进行中问候不判答案）与错答 5 句模板池已部署；72 项测试通过，真实模型表达冒烟通过。
- 已接入 Owner 的人格技能库（`D:/PersonalHub/Bak/apeiria-roleplay`）：`apeiria_core.persona` 加载裁剪组装后注入在场决策与表达层；`persona_dir` 配置可换目录，缺失回退内置；另有游戏表达层（full/lead 两模式）、闲聊分诊、导入一致性 guard 测试；76 项测试通过。
## 明确未做
...
4. 部署在场模型版本到隔离 AstrBot（先完成跨重启验收，再重建 wheels 部署），启动 `python -m apeiria_webui`，在测试群验收：点名回复、话题接话与吐槽、AI 自主出题、静默优先与限频护栏，并在 WebUI 观察实时事件流。
